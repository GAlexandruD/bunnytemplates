...
This Environment [Template](https://documentation.bunnyshell.com/docs/templates-what-are-templates) is a boilerplate for creating a new environment based on Next.js.

The template provides the Bunnyshell configuration composed of 1 Component (NextJS).
...
This Environment Template contains 1 component:

- `next-app` based on a `nextjs` image

and 1 persistent volumes:
...
You can also pass on the Component ID, to skip the wizard. The full command can be copied from the UI: go to the Environment details screen, click the desired Component, then expand the _Component Actions_ and select _Remote Development_.

💡 When typing, you can use the shorter alias: `bns rdev up`

🧱 Please note that starting a Remote Development session requires the _Remote Development_ setting to be _ON_ for the Environment (_ON_ is the default value).
...
- the _remote path_: the path where code files are located within the container (you can find this out from the `Dockerfile`, it's in the `WORKDIR` statement); for the `backend` application, it is `/usr/src/app/backend`.
...
You need to provide the _remote path_: the path where code files are located within the container (you can find this out from the `Dockerfile`, it's in the `WORKDIR` statement); for the `api` application, it is `/usr/src/app/backend`.
...
You can also pass on the Component ID, to skip the wizard. The full command for starting a session can be copied from the UI: go to the Environment details screen, click the desired Component, then expand the _Component Actions_ and select _Remote Development_. Then, you just need to replace `up` with `down.
